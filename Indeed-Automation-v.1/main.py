#!/usr/bin/env python3
"""
Indeed Job Application Automation Agent CLI
Autonomous job-search and application assistant with dedicated Chrome profile integration for Indeed.com.
"""
...
from src.platforms.indeed import IndeedAutomation
...
async def run_automation(source: str = "indeed", keywords: Optional[List[str]] = None, limit: Optional[int] = None, test_mode: bool = False):
    """
    Main orchestration entrypoint for autonomous Indeed job search & application.
    """
    ...
    if test_mode:
        limit = 1
        print("\n🧪 [TEST MODE ACTIVE] Will process and apply to exactly 1 target company on Indeed for verification.")
    ...
    print("\n" + "=" * 65)
    print("🚀 STARTING INDEED JOB APPLICATION AUTOMATION AGENT")
    ...
    from src.notifier import notify_started, notify_completion, notify_stopped, notify_error
    notify_started("Indeed")
    ...
    try:
        print("\n[Agent] Starting Indeed Workflow...")
        ind = IndeedAutomation(cdp, scorer, tracker, profile, settings)
        res = await ind.search_and_process_jobs(keywords=keywords, limit=limit)
        ...
        notify_completion(submitted_count=submitted, total_evaluated=evaluated)

    except asyncio.CancelledError:
        from src.notifier import notify_stopped
        notify_stopped("Indeed automation cancelled by user.")
        raise
...
def review_queue():
    ...
    print(f"\n=======================================================")
    print(f"📝 INDEED REVIEW QUEUE ({len(review_items)} jobs waiting for your decision)")
    print(f"=======================================================")
    if not review_items:
...
def test_score():
    ...
    sample_job = {
        "title": "Software Engineer - SDE 1 (Java / Python Backend)",
        "company": "TechNova Innovations",
        "description": "We are seeking a motivated junior Software Engineer with 0-2 years of experience. Stack: Python, Java, Spring Boot, REST APIs, PostgreSQL, Docker, Git. Remote / Bengaluru, India. Strong foundation in Data Structures, Algorithms, and OOP required.",
        "skills": ["Python", "Java", "Spring Boot", "PostgreSQL", "REST APIs", "Git", "Docker"],
        "location": "Bengaluru, Karnataka / Remote",
        "job_url": "https://in.indeed.com/viewjob?jk=sample123456",
        "source": "Indeed"
    }

    result = scorer.evaluate(sample_job)
    print("\n" + "=" * 60)
    print("🧪 TEST SCORING ENGINE EVALUATION (INDEED)")
    print("=" * 60)
...
def launch_browser():
    import subprocess
    from src.cdp_client import resolve_profile_directory
    settings = load_json("config/settings.json")
    browser_cfg = settings.get("browser", {})
    port = browser_cfg.get("remote_debugging_port", 9222)
    user_data_dir = os.path.abspath(os.path.expanduser(browser_cfg.get("user_data_dir", "~/.config/google-chrome-automation")))
    profile_name = browser_cfg.get("profile_name", "Sakshi-Automation")
    fallback_dir = browser_cfg.get("profile_directory", "Profile 4")
    profile_dir = resolve_profile_directory(user_data_dir, profile_name, fallback=fallback_dir)
...
def main():
    parser = argparse.ArgumentParser(description="Indeed Job Application Automation Agent")
    subparsers = parser.add_subparsers(dest="command", help="Available subcommands")

    # run command
    run_parser = subparsers.add_parser("run", help="Run job discovery and application loop on Indeed")
    run_parser.add_argument("--source", default="indeed", help="Job portal source (default: indeed)")
    ...
    if args.command == "run":
        asyncio.run(run_automation(source="indeed", keywords=args.keywords, limit=args.limit, test_mode=args.test))
