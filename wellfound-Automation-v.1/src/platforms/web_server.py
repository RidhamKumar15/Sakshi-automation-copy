...
async def _run_apply_waiting_task(limit: Optional[int] = None, single_url: Optional[str] = None):
    ...
    try:
        ...
        if single_url:
            ...
            job_dict = {
                "title": job_rec.get("role", "Software Engineer") if job_rec else "Software Engineer",
                "company": job_rec.get("company", "Hiring Startup") if job_rec else "Hiring Startup",
                "job_url": single_url,
                "location": "India",
            ...
            broadcast_log(f"[Genesis] 🏁 Batch apply finished! Submitted: {res.get('submitted')}, Failed: {res.get('failed')}")
