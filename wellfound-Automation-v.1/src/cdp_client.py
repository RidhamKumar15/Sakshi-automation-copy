...
class ChromeCDPClient:
    ...
    def __init__(
        self,
        port: int = 9222,
        user_data_dir: str = "/home/ridhamverma/.config/google-chrome-automation",
        ...
        self.profile_directory = self._resolve_profile_directory(user_data_dir, profile_name, fallback=profile_directory)
    ...
    def _sync_profile_data(self):
        ...
        Profile 4 into the dedicated automation data directory.
        """
        ...
        try:
            ...
            if os.path.exists(p_src):
                ...
                subprocess.run(
                    ["rsync", "-a", "--update", "--exclude=Singleton*", "--exclude=*.lock", "--exclude=LOCK", p_src + "/", p_dst + "/"],
                    capture_output=True
                )
        except Exception as e:
