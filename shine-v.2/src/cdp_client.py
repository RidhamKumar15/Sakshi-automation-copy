...
class ChromeCDPClient:
    ...
    Connects to real Google Chrome with the dedicated 'Sakshi-Automation' profile.
    ...
    def __init__(
        ...
        profile_name: Optional[str] = "Sakshi-Automation"
    ):
        self.port = port
        self.user_data_dir = os.path.abspath(os.path.expanduser(user_data_dir))
        self.profile_name = profile_name
        self.profile_directory = self._resolve_profile_directory(self.user_data_dir, profile_name, fallback=profile_directory)
    ...
    @staticmethod
    def _resolve_profile_directory(user_data_dir: str, profile_name: Optional[str], fallback: str = "Profile 4") -> str:
        if not profile_name:
            return fallback
        # Check source google-chrome and target user_data_dir
        for check_dir in ["~/.config/google-chrome", user_data_dir]:
            try:
                user_data_path = os.path.abspath(os.path.expanduser(check_dir))
                ...
                if os.path.exists(local_state_path):
                    ...
                    for dir_name, info in info_cache.items():
                        ...
                        if (
                            profile_name.lower() == name.lower()
                            or profile_name.lower() == dir_name.lower()
                            or profile_name.lower() in name.lower()
                            or profile_name.lower() in gaia_name.lower()
                            or profile_name.lower() in user_name.lower()
                        ):
                            return dir_name
            except Exception as e:
                logger.warning(f"Failed to auto-resolve profile directory for '{profile_name}' in {check_dir}: {e}")
        return fallback

    def _sync_profile_data(self):
        """
        Syncs candidate cookies, logins, and session from the primary Google Chrome
        Profile into the dedicated automation data directory.
        Preserves existing session and login data in the automation profile.
        """
        src_chrome = os.path.expanduser("~/.config/google-chrome")
        target_dir = os.path.abspath(os.path.expanduser(self.user_data_dir))
        if target_dir == src_chrome:
            return

        try:
            os.makedirs(target_dir, exist_ok=True)
            local_state_src = os.path.join(src_chrome, "Local State")
            ...
            p_src = os.path.join(src_chrome, self.profile_directory)
            p_dst = os.path.join(target_dir, self.profile_directory)
            if os.path.exists(p_src):
                os.makedirs(p_dst, exist_ok=True)
                # If target already has session cookies or storage, preserve them and do not overwrite
                exclude_flags = [
                    "--exclude=Singleton*",
                    "--exclude=*.lock",
                    "--exclude=LOCK"
                ]
                if os.path.exists(os.path.join(p_dst, "Cookies")) or os.path.exists(os.path.join(p_dst, "Local Storage")):
                    exclude_flags.extend([
                        "--exclude=Cookies*",
                ...
                subprocess.run(
                    ["rsync", "-a", "--update"] + exclude_flags + [p_src + "/", p_dst + "/"],
                    capture_output=True
                )
        except Exception as e:
            logger.warning(f"Could not sync profile data: {e}")
    ...
    async def is_chrome_running(self) -> bool:
        ...
        try:
            async with aiohttp.ClientSession(trust_env=False) as temp_session:
    ...
    def launch_chrome_process(self) -> Optional[subprocess.Popen]:
        ...
        user_data_path = os.path.abspath(os.path.expanduser(self.user_data_dir))
        candidates = [
            "google-chrome",
        ...
        for binary in candidates:
            ...
            try:
                ...
                print(f"[CDP] Launched Chrome using '{binary}' with profile '{self.profile_directory}' ({self.profile_name or 'Default'})")
                return proc
            except Exception:
                continue

        print(f"[CDP] Could not auto-launch Chrome. Please start Chrome with:\n  google-chrome --user-data-dir=\"{user_data_path}\" --remote-debugging-port={self.port} --profile-directory=\"{self.profile_directory}\"")
        return None

    async def connect(self, target_url_match: Optional[str] = "shine.com", create_new: bool = False) -> bool:
        """
        Connects to Chrome CDP endpoint and attaches to an active tab.
        Prioritizes reusing existing Shine or active tabs to avoid opening unwanted new tabs.
        """
        ...
        # Priority 1: Check if an existing tab matches target_url_match (e.g. shine.com)
        if target_url_match:
            for t in targets:
                if t.get("type") == "page":
                    url = t.get("url", "")
                    if target_url_match in url and not any(ph in url for ph in protected_hosts):
                        page_target = t
                        print(f"[CDP] 🎯 Found existing portal tab matching '{target_url_match}': {t.get('title')} ({url})")
                        break

        # Priority 2: Check if previously attached tab is still active
        if not page_target and self.current_target_id:
            for t in targets:
                if t.get("type") == "page" and t.get("id") == self.current_target_id:
                    url = t.get("url", "")
                    if not any(ph in url for ph in protected_hosts):
                        page_target = t
                        print(f"[CDP] 🔄 Reusing active automation tab: {t.get('title')} ({url})")
                        break

        # Priority 3: Reuse any existing blank or new tab
        if not page_target:
            for t in targets:
                if t.get("type") == "page":
                    url = t.get("url", "")
                    if url in ("about:blank", "chrome://newtab", "chrome://newtab/") and not any(ph in url for ph in protected_hosts):
                        page_target = t
                        print(f"[CDP] 📄 Reusing existing blank tab: {url}")
                        break

        # Priority 4: Reuse any non-protected page tab if not forcing create_new
        if not page_target and not create_new:
            for t in targets:
                if t.get("type") == "page":
                    url = t.get("url", "")
                    if not any(ph in url for ph in protected_hosts):
                        page_target = t
                        print(f"[CDP] 🌐 Reusing existing browser tab: {t.get('title')} ({url})")
                        break

        # Priority 5: Only open a new tab if no suitable candidate tab exists
        if not page_target:
            print("[CDP] ➕ Opening dedicated automation tab...")
        ...
        print(f"[CDP] Attaching to tab: {page_target.get('title')} ({page_target.get('url')})")
    ...
    async def check_for_security_challenge(self) -> Dict[str, Any]:
        ...
        - Visible reCAPTCHA / hCaptcha / Arkose puzzle popups
        - Visible OTP / 2FA verification inputs
        - Visible login gates (when session is expired or logged out)
        """
        js = """
        (() => {
            ...
            // 4. Login Required Gate (Check for Shine or portal login gates)
            const login_required = (() => {
                const hasLoginModal = document.querySelector('[class*="login-modal"], form[action*="login"], div[class*="login-layer"], div[class*="loginModal"]');
                if (hasLoginModal && hasLoginModal.offsetParent !== null) return true;

                if (window.location.href.includes('/myshine/login') || window.location.href.includes('/login') || window.location.href.includes('/signup')) {
                    const hasProfile = document.querySelector('[class*="profile"], [class*="userAvatar"], header [class*="user"], nav [class*="user"]');
